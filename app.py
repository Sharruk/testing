...
# Flask-Login configuration with secure cookies
app.config['REMEMBER_COOKIE_DURATION'] = timedelta(days=int(os.environ.get('VERIFICATION_INTERVAL_DAYS', '90')) * 2)  # Remember longer than verification period
app.config['REMEMBER_COOKIE_SECURE'] = os.environ.get('FLASK_ENV') == 'production'  # Secure cookies in production
app.config['REMEMBER_COOKIE_HTTPONLY'] = True
app.config['REMEMBER_COOKIE_SAMESITE'] = 'Lax'
app.config['SESSION_COOKIE_SECURE'] = os.environ.get('FLASK_ENV') == 'production'
app.config['SESSION_COOKIE_HTTPONLY'] = True
app.config['SESSION_COOKIE_SAMESITE'] = 'Lax'
app.config['PERMANENT_SESSION_LIFETIME'] = timedelta(days=int(os.environ.get('VERIFICATION_INTERVAL_DAYS', '90')))
...
# Direct login not required - using OTP verification for uploads
login_manager.login_message = 'Please verify your email to access this page.'
...
def guest_or_contributor_required(f):
    ...
    def decorated_function(*args, **kwargs):
        if not current_user.is_authenticated:
            flash('Please verify your email to access this page.', 'info')
            return redirect(url_for('upload_service'))
        return f(*args, **kwargs)
    return decorated_function

# Register Google OAuth blueprint
# Google OAuth integration removed - using OTP verification instead
...
def logout():
    ...
    if current_user.is_authenticated:
        ...
        # Clear the user from Flask-Login
        logout_user()
        # Clear all session data for complete logout
        session.clear()
        
        # Create response to clear remember cookies
        ...
        flash(f'{user_name} has been logged out successfully.', 'info')
        return response
    return redirect(url_for('index'))

@app.route('/guest-login', methods=['GET', 'POST'])
def guest_login():
    """Guest login for any email address (non-SSN emails get guest role)"""
    if current_user.is_authenticated:
    ...
    if request.method == 'POST':
        ...
        try:
            # Check if user already exists (only check regular email, NOT ssn_email)
            ...
            if not user:
                ...
                # Existing non-SSN user
                # SECURITY: Double-check they're not a contributor (should not happen, but extra safety)
                if user.is_contributor:
                    flash('This account requires SSN verification. Please use the contributor login.', 'warning')
                    return redirect(url_for('upload_service'))
                flash_message = f'Welcome back, {user.name}!'
            
            # Log in the user (only non-SSN guests at this point)
            ...
            app.logger.error(f"Error in guest login for {email}: {str(e)}")
...
def quick_login():
    """Universal login - handles both guest and contributor scenarios"""
    if current_user.is_authenticated:
        flash('You are already logged in!', 'success')
        return redirect(url_for('index'))
    ...
    if request.method == 'POST':
        ...
        try:
            # Check for existing user - contributors stored by ssn_email, others by email
            if email.endswith('@ssn.edu.in'):
                user = User.query.filter_by(ssn_email=email).first()
            else:
                user = User.query.filter_by(email=email).first()
            ...
            if user:
                ...
                if user.is_contributor and not user.is_verification_expired():
                    ...
                    # Contributor with expired verification
                    flash('Your SSN email verification has expired. Please re-verify to continue.', 'warning')
                    return redirect(url_for('upload_service'))
                    
                else:
                    # Guest user - redirect to guest login for proper authentication
                    flash('Please complete the login process to continue.', 'info')
                    session['pre_fill_email'] = email
                ...
                if email.endswith('@ssn.edu.in'):
                    ...
                    flash('Please complete guest registration to continue.', 'info')
                    session['pre_fill_email'] = email
                    return redirect(url_for('guest_login'))
                    
        except Exception as e:
            app.logger.error(f"Error in quick login for {email}: {str(e)}")
            flash('An error occurred during login. Please try again.', 'error')
    
    return render_template('quick_login.html')

@app.route('/clear-session-email', methods=['POST'])
def clear_session_email():
    """Clear pre-filled email from session"""
    session.pop('pre_fill_email', None)
    return '', 204
...
def upload_service():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            existing_user = User.query.filter_by(ssn_email=email).first()
            if existing_user:
                # If it's a different user's email, block access
                if current_user.is_authenticated and existing_user.id != current_user.id:
                    flash('This SSN email is already verified by another account.', 'error')
                    return render_template('upload_service.html')
                # If verification expired, continue with re-verification flow
                elif existing_user.is_verification_expired():
            ...
            if email_service.send_otp_email(email, otp, user.name):
                # Store upload session data
                session['upload_pending'] = True
                session['upload_email'] = email
                session['upload_user_id'] = user.id
                
                flash('Verification code sent to your email. Please check and enter it below.', 'success')
                return redirect(url_for('upload_service_verify'))
            else:
                user.clear_otp()
                db.session.commit()
            ...
            app.logger.error(f"Error in upload service for {email}: {str(e)}")
            flash('An error occurred. Please try again.', 'error')
    
    return render_template('upload_service.html')

@app.route('/upload-service/verify', methods=['GET', 'POST'])
def upload_service_verify():
    """Verify OTP for upload service"""
    if 'upload_pending' not in session:
        flash('Please start the upload process again.', 'error')
        return redirect(url_for('upload_service'))
    ...
    if request.method == 'POST':
        ...
        try:
            user_id = session.get('upload_user_id')
            ...
            if user.is_otp_locked_out():
                lockout_time = user.otp_lockout_until.strftime('%H:%M')
                flash(f'Too many failed attempts. Please try again after {lockout_time}.', 'error')
                return render_template('upload_service_verify.html')
            ...
            if user.verify_otp(otp):
                ...
                session.pop('upload_email', None) 
                session.pop('upload_user_id', None)
                
                # Make session truly persistent and mark login
                session.permanent = True
                user.mark_login()
                db.session.commit()
                ...
                if remaining > 0:
                    flash(f'Invalid verification code. {remaining} attempts remaining.', 'error')
                else:
...
@app.route('/upload-service/resend-otp', methods=['POST'])
def upload_service_resend_otp():
    """Resend OTP for upload service with cooldown protection"""
    if 'upload_pending' not in session:
        return jsonify({'success': False, 'message': 'Please start the upload process again.'})
    ...
    try:
        ...
        # Check cooldown - prevent resend within 2 minutes of last attempt  
        last_resend = session.get('last_upload_otp_resend')
        if last_resend:
            from datetime import datetime as dt
            last_time = dt.fromisoformat(last_resend)
            if (datetime.utcnow() - last_time).seconds < 120:  # 2 minutes cooldown
                remaining = 120 - (datetime.utcnow() - last_time).seconds
                return jsonify({'success': False, 'message': f'Please wait {remaining} seconds before requesting another code.'})
        
        # Generate new OTP
        email = session['upload_email']
        otp = user.generate_otp()  # This resets attempts automatically
        db.session.commit()
        
        if email_service.send_otp_email(email, otp, user.name):
            # Record resend time for cooldown
            session['last_upload_otp_resend'] = datetime.utcnow().isoformat()
        ...
        app.logger.error(f"Error resending upload OTP: {str(e)}")
...
ALLOWED_EXTENSIONS = {'pdf', 'doc', 'docx', 'ppt', 'pptx', 'txt', 'jpg', 'jpeg', 'png', 'zip', 'rar'}  # Secure file types
...
def vote_file(file_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'likes': found_file['likes'],
            'dislikes': found_file['dislikes']
        })
...
def add_comment(file_id):
    ...
    try:
        ...
        return jsonify({
            'success': True,
            'comment': new_comment,
            'total_comments': len(found_file['comments'])
        })
...
def load_reports_data():
    ...
    try:
        if os.path.exists('reports.json'):
            ...
            return {"reports": {}, "report_threshold": 3}  # Hide after 3 reports
    except Exception as e:
        app.logger.error(f"Error loading reports data: {str(e)}")
        return {"reports": {}, "report_threshold": 3}
...
def is_file_hidden(file_id, file_type='QP'):
    ...
    try:
        ...
        if file_key in reports_data["reports"]:
        ...
        app.logger.error(f"Error checking file visibility: {str(e)}")
        return False

@app.route('/report_file/<int:file_id>', methods=['POST'])
def report_file(file_id):
    """Handle file reporting"""
    try:
        data = request.get_json()
        file_type = data.get('file_type', 'QP')
        ...
        if is_hidden:
            ...
            response_data['message'] = f'Report submitted successfully. File will be hidden after {threshold - report_count} more report(s).'
...
def file_detail(file_id):
    ...
    try:
        ...
        # Check if file is hidden due to reports
...
@app.route('/verify-ssn-email', methods=['GET', 'POST'])
@login_required
def verify_ssn_email():
    """Request SSN email verification"""
    if current_user.is_contributor:
        flash('You already have contributor access.', 'info')
        return redirect(url_for('index'))
    
    if request.method == 'POST':
        ssn_email = request.form.get('ssn_email', '').strip().lower()
        
        # Validate SSN email format
        if not ssn_email.endswith('@ssn.edu.in'):
            flash('Please enter a valid SSN email address (@ssn.edu.in).', 'error')
            return render_template('verify_ssn_email.html')
        
        # Check if email is already verified by another user
        existing_user = User.query.filter_by(ssn_email=ssn_email).first()
        if existing_user and existing_user.id != current_user.id:
            flash('This SSN email is already verified by another user.', 'error')
            return render_template('verify_ssn_email.html')
        ...
        try:
            ...
            # Send OTP email
            if email_service.send_otp_email(ssn_email, otp, current_user.name):
                session['pending_ssn_email'] = ssn_email
                flash('OTP sent to your SSN email. Please check your inbox.', 'success')
            ...
            app.logger.error(f"Error sending OTP to {ssn_email}: {str(e)}")
...
@login_required
def verify_otp():
    """Verify OTP and promote to contributor"""
    if current_user.is_contributor:
        flash('You already have contributor access.', 'info')
        return redirect(url_for('index'))
    
    if 'pending_ssn_email' not in session:
    ...
    if request.method == 'POST':
        ...
        try:
            # Check if user is locked out
            if current_user.is_otp_locked_out():
                lockout_time = current_user.otp_lockout_until.strftime('%H:%M')
                flash(f'Too many failed attempts. Please try again after {lockout_time}.', 'error')
                return render_template('verify_otp.html')
            
            if current_user.verify_otp(otp):
                # OTP is valid, mark user as verified contributor with persistent session
                ssn_email = session['pending_ssn_email']
                current_user.mark_verified(ssn_email)
                db.session.commit()
                
                # Create persistent login session with remember cookie
                login_user(current_user, remember=True)
                session.permanent = True
                
                # Clear session data
                session.pop('pending_ssn_email', None)
                
                # Send success email
                email_service.send_verification_success_email(ssn_email, current_user.name)
                
                flash(f'Congratulations! You now have contributor access. Welcome to the team!', 'success')
                return redirect(url_for('index'))
            else:
                db.session.commit()  # Save failed attempt count
            ...
            app.logger.error(f"Error verifying OTP for user {current_user.id}: {str(e)}")
...
@login_required
def resend_otp():
    """Resend OTP to SSN email with cooldown protection"""
    if current_user.is_contributor:
        return jsonify({'success': False, 'message': 'You already have contributor access.'})
    
    if 'pending_ssn_email' not in session:
        return jsonify({'success': False, 'message': 'Please start the verification process again.'})
    
    # Check cooldown - prevent resend within 2 minutes of last attempt
    last_resend = session.get('last_otp_resend')
    if last_resend:
        from datetime import datetime as dt
        last_time = dt.fromisoformat(last_resend)
        if (datetime.utcnow() - last_time).seconds < 120:  # 2 minutes cooldown
    ...
    try:
        ssn_email = session['pending_ssn_email']
        otp = current_user.generate_otp()  # This resets attempts automatically
        db.session.commit()
        
        if email_service.send_otp_email(ssn_email, otp, current_user.name):
            # Record resend time for cooldown
            session['last_otp_resend'] = datetime.utcnow().isoformat()
            return jsonify({'success': True, 'message': 'New verification code sent successfully.'})
        else:
            current_user.clear_otp()
            db.session.commit()
