...
def create_tables():
    """Create all database tables (safe for existing tables)"""
    ...
    with app.app_context():
        try:
            ...
            print(f"Tables already exist or error creating tables: {str(e)}")
...
def seed_subjects():
    ...
    with app.app_context():
        ...
        if existing_count > 0:
            print(f"Database already has {existing_count} subjects, skipping seeding...")
            return
...
def create_admin_user():
    ...
    # Only create admin user if explicitly enabled and not in production
    if not os.environ.get('ENABLE_SEED_ADMIN') or os.environ.get('FLASK_ENV') == 'production':
        print("Admin user creation skipped (production or not enabled)")
        return
    ...
    with app.app_context():
        ...
        # Use password from environment or skip creation
...
def migrate_existing_files():
    """Migrate existing JSON file data to database (safe for repeated runs)"""
    ...
    with app.app_context():
        ...
        existing_db_files = File.query.count()
        if existing_db_files > 0:
            print(f"Database already has {existing_db_files} files, skipping migration...")
            return
