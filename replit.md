...
- September 12, 2025. **MAJOR**: Replaced Google OAuth domain-based role assignment with secure email-based OTP verification system for @ssn.edu.in addresses - implemented 6-digit OTP generation with SHA256 hashing and 10-minute expiry, added Flask-Mail email service with professional HTML/text templates, created comprehensive rate limiting (5 failed attempts trigger 15-minute lockout), added unique SSN email constraint, implemented resend cooldown protection, updated UI with verification flow and status badges, migrated from email domain detection to database-based role management for enhanced security
