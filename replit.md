...
- November 15, 2025. **Navigation Enhancement for Contributor-Trusted Workflow**: Enhanced user navigation to surface management pages based on user roles - updated base.html to add role-based navigation links in user dropdown menu (Admin: Dashboard, Verify Files, Reported Files, Manage All Uploads; Contributor: Manage My Uploads when verified), updated admin dashboard quick actions section to highlight the three new management pages (Verify Files, Reported Files, Manage Uploads) plus Upload Files for improved discoverability, all navigation changes align with existing backend routes and role-based access controls, improving user experience for the contributor-trusted workflow system
- November 12, 2025. **Contributor-Trusted Workflow System**: Implemented comprehensive role-based file visibility and verification workflow - added is_file_visible_to_user() helper function to enforce visibility rules (guests see only verified files, contributors and admins see all files), updated upload route to save files to both JSON and PostgreSQL database with verified=False by default, updated category_view, api_search, and search routes to use database queries with role-based filtering, added fallback to JSON data with same filtering logic for legacy data, configured PostgreSQL database with File, User, Subject, and Report models, integrated admin verification pages (/admin/verify-files) for reviewing and verifying uploaded files, integrated admin reported files page (/admin/reported-files) for reviewing user reports, updated manage_uploads page with report functionality allowing contributors to flag inappropriate content, added secure API endpoints (/api/files/verify, /api/files/delete, /api/files/report, /api/reports, /api/reports/review) with proper CSRF protection and role-based authentication, ensured all file listing routes filter based on verified status and user role for secure content management
